...
/// Everything the rest of the app needs a handle on. `window` and
/// `version_label` are held rather than read: dropping the window would tear
/// down a live control tree, and the version label never changes with
/// `UiState`. `actions` has to stay alive too, since the table's data
/// source/delegate and every control's target are unretained references to it.
pub struct Views {
    #[allow(dead_code)]
...
/// Builds every view, wiring `Actions` in as the target/action or
/// data-source/delegate of every control.
pub fn build(mtm: MainThreadMarker) -> Views {
    ...
    // The banner sits above the detail pane rather than inside it, so neither
    // `detail.rs` nor `window.rs` needs to know it exists.
...
///
/// Deliberately not incremental: at this control count re-setting everything
/// is free, and it means there is exactly one place where a control's
...
pub fn apply(state: &UiState, views: &Views) {
    ...
    // A click already leaves the sidebar's selection where it belongs, but
    // restoring the last-selected game from preferences never goes through
    // one. Through `Actions` rather than straight at the table: AppKit sends
    // `tableViewSelectionDidChange:` for a programmatic selection too, so a
    // bare `selectRowIndexes` here would post a `SelectGame` out of the middle
    // of a render and never stop.
    if let Some(row) = sidebar::row_for_game(state.selected_game) {
        views.actions.select_sidebar_row(&views.sidebar_table, row);
    }

    // Which entry is active is resolved once, through `active_entry`, the one
    // place that knows `VersionStore::active` reports `Installed::name` in
    // multi-version mode but `Installed::tag` in compatible mode.
    ...
    // The button says what is happening only while something reports a phase.
    // `busy.status` is `None` between initiating an operation and its first
    // progress report, and for the whole of one that reports none, so the
    // button keeps its ordinary title rather than claiming a download that is
    // not running.
    ...
    // Which rows are relevant follows the selected game, not the list of
    // original games: RollerCoaster Tycoon 1 and 2 are both read by OpenRCT2,
    // and Locomotion's row beside them would offer an install that the game in
    // front of the user cannot use.
    for row in &views.game_data {
        ...
        row.path
            .setStringValue(&NSString::from_str(&match installed {
                ...
                // An optional one says so: "not installed" beside a game that
                // will not start without it means something different.
    ...
    match &state.error {
        Some(alert) => {
            ...
            // title with a `{0}` in it. `state.rs` stays free of `strings.rs`,
            // so it hands over the raw argument rather than formatted text.
            let title = match &alert.title_arg {
                Some(arg) => strings::format1(&alert.title, arg),
                None => strings::get(&alert.title),
            };
            // The message is a worker's own words -- the store's, GitHub's or
            // the OS's, none localizable -- so it is shown verbatim unless the
            // reducer raised the error itself and named a key instead.
